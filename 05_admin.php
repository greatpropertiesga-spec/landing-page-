...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .s-logo small{display:block;color:#555;font-size:11px;
    ...
    .nav-a:hover,.nav-a.on{background:#1a1a1a;color:#fff;
                             border-left-color:#cc0000}
    .s-foot{padding:16px 20px;border-top:1px solid #1e1e1e;
            font-size:12px;color:#555}
    .s-foot a{color:#777;text-decoration:none}
    .s-foot a:hover{color:#fff}
    .main{margin-left:220px;min-height:100vh}
    .topbar{background:#fff;padding:15px 26px;
    ...
    .alert{background:#fff0f0;border:1px solid #fca5a5;color:#b00;
           padding:14px 18px;border-radius:8px;margin-bottom:18px;
           font-size:13px;line-height:1.6}
    .alert code{background:#ffe;padding:2px 6px;border-radius:4px;
                font-size:12px;display:block;margin-top:8px;
                word-break:break-all;color:#555}
    .setup-box{background:#fff;border:2px dashed #ffd700;border-radius:12px;
               padding:28px;margin-bottom:20px;text-align:center}
    .setup-box h2{font-size:18px;margin-bottom:10px;color:#111}
    .setup-box p{color:#666;font-size:14px;line-height:1.7}
    .setup-box a{color:#cc0000;font-weight:bold}
    ...
    .ti{flex:1 1 180px;padding:9px 12px;border:1.5px solid #ddd;
        border-radius:7px;font-size:14px}
    .ti:focus{outline:none;border-color:#cc0000}
    .tsel{padding:9px 12px;border:1.5px solid #ddd;
          border-radius:7px;font-size:14px;background:#fff}
    .btn-s{background:#cc0000;color:#fff;border:none;
           padding:9px 18px;border-radius:7px;
           font-size:14px;font-weight:bold;cursor:pointer}
    .btn-c{background:#f0f0f0;color:#555;border:none;
    ...
    th{padding:11px 14px;text-align:left;font-size:11px;
    ...
    .bx{background:#dcfce7;color:#166534}
    .bl{background:#f3f4f6;color:#6b7280}
    ...
    .sfb{background:#cc0000;color:#fff;border:none;
         padding:4px 8px;border-radius:5px;font-size:11px;cursor:pointer}
    .db{background:none;border:none;color:#ddd;font-size:15px;
...
<div class="sidebar">
  <div class="s-logo">Great <span>Properties</span> GA
...
<div class="main">
  ...
  <div class="content">
    ...
    <?php if (SB_KEY === 'YOUR_SUPABASE_SERVICE_ROLE_KEY'): ?>
    <div class="setup-box">
      <h2>&#9888; Falta configurar Supabase</h2>
      <p>
        El admin necesita tu <strong>Service Role Key</strong> de Supabase.<br>
        Ve a <a href="https://supabase.com" target="_blank">supabase.com</a>
        &rarr; tu proyecto &rarr; <strong>Settings &rarr; API</strong><br>
        Copia el key <strong>service_role</strong> y p&eacute;galo en
        <code>02_config.php</code> reemplazando <code>YOUR_SUPABASE_SERVICE_ROLE_KEY</code>.
      </p>
    </div>
    <?php elseif (!$sb_ok): ?>
    <div class="alert">
      &#9888; Error conectando con Supabase (HTTP <?= $res['code'] ?>).
      <code><?= htmlspecialchars(substr($sb_err, 0, 300)) ?></code>
    </div>
    <?php endif; ?>
    ...
    <div class="toolbar">
      <form method="GET" action="admin.php">
        <input class="ti" type="text" name="q"
      ...
      <table>
        ...
        <?php else:
          foreach ($leads as $row):
            $s = $row['status'] ?? 'New';
            $b = match($s) {
              'Contacted' => 'bc', 'Closed' => 'bx', 'Lost' => 'bl', default => 'bn'
            };
        ?>
          <tr>
            <td style="color:#ccc;font-size:12px">#<?= (int)$row['id'] ?></td>
            <td class="tn"><?= htmlspecialchars($row['name'] ?? '') ?></td>
            <td class="tp"><a href="tel:<?= htmlspecialchars($row['phone'] ?? '') ?>">
              <?= htmlspecialchars($row['phone'] ?? '') ?></a></td>
            <td><?= htmlspecialchars($row['email'] ?? '') ?></td>
            <td class="ta" title="<?= htmlspecialchars($row['address'] ?? '') ?>">
            ...
            <td class="td"><?= date('M j, Y g:i a', strtotime($row['created_at'] ?? 'now')) ?></td>
            <td>
